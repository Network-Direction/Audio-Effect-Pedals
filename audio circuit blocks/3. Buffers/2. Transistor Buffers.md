...
Is this a good impedance? It's ok. In the ideal buffer, we want to see an impedance of about 1MΩ or more.
...
JFET buffers follow a similar architecture to the BJT buffer we've already seen. This example is a basic JFET buffer from **Jack Orman**.

There is an old Wah pedal called the **Doctor Q**. It originally came with no input buffer, so Mr Orman designed this buffer to augment the circuit (links in the reference area below).

This architecture, when used with a JFET, is called a **common drain amplifier**.

![image](https://github.com/user-attachments/assets/ca21750c-39aa-4df0-84f2-1cc0faf7fd26)

</br></br>
> [!IMPORTANT]  
> For this JFET, Pin 1 is _drain_, pin 2 is _source_, and pin 3 is _gate_


</br></br>
## Similarities

As before we have the two power supplies, one at 9v, and the other at half that value, at 4.5v. The 4.5v supply typically comes from a 9v supply connected to a voltage divider.

We also see the capacitors at the input and output of the buffer, which are there to remove DC bias voltage.

This has a high input impedance, which is effectively controlled by R1. As this is a 1MΩ resistor, this will have an input impedance of about 1MΩ as well. This is a good (high) input impedance, but could even be increased if needed.

R2 is on the **source** pin of the JFET. It's value can be in the 3.3K to 10K range.


</br></br>
## JFET Selection

Does the choice of JFET matter? Not as much as you'd think. The one listed here is an **MPF102**. The designer suggests a **2N5457** would be a suitable alternative, or nearly any commonly available JFET.

The most important factor is that the JFET is _low noise_, which is a common feature of JFETs anyway.

> [!IMPORTANT]  
> Different JFETs may have different pinouts. If you change the JFET, make sure you check the source and drain pins.


# JFET or BJT?

So which is better? That may come down to preference. As you can see they're nearly almost identical.

I will mention early on that it's getting harder to find JFETs are through-hold packages. They're increasingly becoming available as SMTs only, which might influnce your decision.

JFETs provide an extremely high input impedance (easily in the MΩ range). BJT's can still be high, but not as high as JFET (tens of KΩ to about 1MΩ). However, BJT's are capable of providing a lower output impedance. For a buffer through, a high input impedance is more important.

They are biassed differently too. BJT's are _current controlled_ devices, while JFETs are _voltage controlled_. The result is that JFETs use less power then a BJT.

In general, JFETs are a better choice for buffers, where we need to match input impedances. This is not always true with other types of amplifiers though.

Or, as an alternative, why not try a [JFET based opamp](https://github.com/Network-Direction/Audio-Effect-Pedals/blob/Wha-Pedals/audio%20circuit%20blocks/3.%20Buffers/3.%20Opamp%20Buffers.md)? They have all the advantages of JFETs, but are much easier to set up.
